...
The standard validation workload contains 64 concurrent requests. Each request
has exactly 4096 input tokens and 256 output tokens. A complete warmup batch finishes before
...
bash tools/graph_operator_profile/profile_request.sh \
  deepseek_v4_flash \
  tools/graph_operator_profile/deepseek_v4_flash_request_4096_256.json
```

Results:

```text
/vllm-workspace/graph_operator_profile_runs/deepseek_v4_flash_4096_256/results/
```
...
```text
/vllm-workspace/graph_operator_profile_runs/deepseek_v4_flash_eager_4096_256/results/
```
...
```text
/vllm-workspace/graph_operator_profile_runs/qwen3_6_35b_a3b_graph_4096_256/results/
```
...
```text
/vllm-workspace/graph_operator_profile_runs/qwen3_6_35b_a3b_eager_4096_256/results/
```
...
## Reproduce the 4096/256 four-scenario validation

Use one model configuration for all four rows:
...
Run each model with these four environments and unique suffixes:

| Scenario | Server environment |
|---|---|
| plugin graph | default |
| plugin eager | `PROFILE_EXECUTION_MODE=eager` |
| native graph | `VLLM_PLUGINS=""` |
| native eager | `VLLM_PLUGINS="" PROFILE_EXECUTION_MODE=eager` |

Start the selected server, wait for `/health`, and run the matching request:

```bash
<server-environment> \
PROFILE_RUN_SUFFIX=<unique-suffix> \
...
For plugin runs, verify that `/tmp/flaggems_enable_oplist.txt` was modified by
the current run, then move it into the run's `results` directory. For native
runs, verify that the API-server process environment contains the explicit
empty `VLLM_PLUGINS` value, the log has no plugin activation message, and no
FlagGems oplist is generated.

Use `compare_kernel_profiles.py --scan-cpu-operators` for these four pairs:
...
Place the four outputs under one comparison directory with subdirectories
`plugin_vs_native_graph`, `plugin_vs_native_eager`,
`plugin_graph_vs_eager`, and `native_graph_vs_eager`. Then generate the
consolidated report:
